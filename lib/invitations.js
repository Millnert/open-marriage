...
exports.loadInvitationsWithSQL = loadInvitationsWithSQL;
exports.updateInvitation       = updateInvitation;
exports.addGuestToInvitation   = addGuestToInvitation;
...
function loadInvitationsWithSQL(invitationsql, callback) {
    function processInvitation(db, invitation, callback) {
        db.query(INVITATION_GUESTS, [invitation.id], function (err, results) {
            if (err) { return callback(err); }
    ...
    pg.connect(config.database, function (err, db, done) {
        ...
        async.waterfall([
            db.query.bind(db, invitationsql),
            processInvitations.bind(null, db)
        ], function () {
            done();
            callback.apply(null, arguments);
...
function addGuestToInvitation(id, callback) {
    var values    = [id],
	chk_query = INVITATION_BY_ID;
	ins_query = ADDGUEST_TO_INV;

    dbhelp.runQuery(chk_query, values, function(err, invitation) {
        if (err) { return callback(err); }
	...
	if (!invitation.allow_plusone) {
        ...
        dbhelp.runQuery(ins_query, values, function(err, result) {
            if (err) { return callback(err); }
